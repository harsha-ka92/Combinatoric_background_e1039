...
{ 
  ...
  /*if (! gSystem->AccessPathName("opts/run6.opts")) { // found
    cout<<"Using: opts/ opt file" <<endl; 
    //rc->get_CharFlag("opts");
    
  } else {
    cout<<"Using: $ktracker opt file" <<endl;
    //JobOptsSvc::instance()->init("$KTRACKER_ROOT/opts/mc.opts");
    //JobOptsSvc::instance()->init("/e906/app/users/liuk/GridOperation/R008/opts/run6.opts");
  }
 */
  ...
  for(int i_evt=0;i_evt<input_tree->GetEntries();i_evt++) 
    {
      ...
      //if (pos_tracks_input->size() !=0 || neg_tracks_input->size() !=0){
        //std::cout<<"# of pos tracks : " << pos_tracks_input->size() <<", # of neg tracks : " << neg_tracks_input->size()<<std::endl;
      //}
  ...
  for (auto iter = neg_tracks_hold.begin(); iter != neg_tracks_hold.end(); iter++) {
	  ...
	  for (std::size_t k=0; k<neg_tracks_hold.size(); ++k)
		...
		if(vtxfit->processOneDimuon(track1, track2, recDimuon)){
			...
			recDimuon.set_track_id_pos(trackIDs1[j]);
			recDimuon.set_track_id_neg(trackIDs2[k]);
