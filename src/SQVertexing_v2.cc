...
namespace
{
  ...
  void initGlobalVariables()
  {
	...
	std::cout<<"Z_UPSTREAM :" <<Z_UPSTREAM<<std::endl;
	std::cout<<"Z_DUMP :" <<Z_DUMP<<std::endl;
...
SQVertexing_v2::SQVertexing_v2(const std::string& name, int sign1, int sign2):
  SubsysReco(name),
...
int SQVertexing_v2::Init(PHCompositeNode* topNode)
{
  initGlobalVariables();
  return Fun4AllReturnCodes::EVENT_OK;
}

int SQVertexing_v2::InitRun(PHCompositeNode* topNode)
{
...
int SQVertexing_v2::process_event(PHCompositeNode* topNode)
{
  ...
  if(Verbosity() > 10) std::cout << "SQVertexing_v2::process_event():  nTracks = " << nTracks << std::endl;
  ...
  for(std::size_t i = 0; i < trackIDs1.size(); ++i)
  {
    for(std::size_t j = charge1 == charge2 ? i+1 : 0; j < trackIDs2.size(); ++j)
    {
...
int SQVertexing_v2::End(PHCompositeNode* topNode)
{
  return Fun4AllReturnCodes::EVENT_OK;
}
...
double SQVertexing_v2::swimTrackToVertex(SQGenFit::GFTrack& track, double z, TVector3* pos, TVector3* mom)
{
  double chi2 = track.swimToVertex(z, pos, mom);
  return chi2;
}

double SQVertexing_v2::refitTrkToVtx(SQGenFit::GFTrack& track, double z, TVector3* pos, TVector3* mom)
{
  if(Verbosity() > 20) std::cout << "SQVertexing_v2::refitTrkToVtx(): z = " << z << ", pos = (" << pos->X() << ", " << pos->Y() << ", " << pos->Z() << "), mom = (" << mom->X() << ", " << mom->Y() << ", " << mom->Z() << ")" << std::endl;
  gfield->setOffset(0.);
  ...
  while(fabs(z_offset_curr - z_offset_prev) > 0.1)
  {
    ...
    if (++n_iter == n_iter_max) {
      std::cout << "!WARNING!  SQVertexing_v2::refitTrkToVtx():  Give up the iteration." << std::endl;
      break;
    }
  }
...
double SQVertexing_v2::refitTrkToVtx(SRecTrack* track, double z, TVector3* pos, TVector3* mom)
{
...
bool SQVertexing_v2::processOneDimuon(SRecTrack* track1, SRecTrack* track2, SRecDimuon& dimuon)
{
  if(Verbosity() > 10) std::cout << "  SQVertexing_v2::processOneDimuon(): " << std::endl;
...
double SQVertexing_v2::findDimuonZVertex(SRecDimuon& dimuon, SQGenFit::GFTrack& track1, SQGenFit::GFTrack& track2)
{
...
/*int SQVertexing_v2::InitField(PHCompositeNode* topNode)
{
  ...
  {
    std::cout << "SQVertexing_v2::InitGeom - Caught an exception " << std::endl;
    gfield = nullptr;
  }
  ...
  if(gfield == nullptr)
  {
    ...
    std::cout << "SQVertexing_v2::InitGeom - creating new GenFit field map" << std::endl;
    gfield = new SQGenFit::GFField(phfield);
  }
  else
  {
    std::cout << "SQVertexing_v2::InitGeom - reading existing GenFit field map" << std::endl;
  }
...
{
  std::cout<<"setting up the B field and storing in 'gfiled'"<<std::endl;
  try
  {
    gfield = dynamic_cast<SQGenFit::GFField*>(genfit::FieldManager::getInstance()->getField());
  }
  ...
  if(gfield == nullptr)
  {
    ...
    std::unique_ptr<PHFieldConfig> default_field_cfg(new PHFieldConfig_v3(rc->get_CharFlag("fMagFile"), rc->get_CharFlag("kMagFile"), rc->get_DoubleFlag("FMAGSTR"), rc->get_DoubleFlag("KMAGSTR"), 5.));
    PHField* phfield = PHFieldUtility::BuildFieldMap(default_field_cfg.get(), 0);

    std::cout << "SQVertexing_v2::InitField - creating new GenFit field map" << std::endl;
...
/*int SQVertexing_v2::InitGeom(PHCompositeNode* topNode)
{
  ...
  {
    std::cout << "SQVertexing_v2::InitGeom - rely on existing TGeo geometry" << std::endl;
  }
